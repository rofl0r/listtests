...
static inline void test_single(int max, int blocks, int blocktimes) {
	...
	o_0 = 0;
	binlist_init(l, sizeof(struct test1), blocks);
	for(i = 0; i < max * blocktimes; i++) {
		...
		binlist_add(l, make_struct(temp, o_0++, disturber));
	}
	o_0 = 0;
	for(i = 0; i < max * blocktimes; i++) {
		assert((O_o = (struct test1*) binlist_get(l, o_0/2)) && O_o->x == o_0++ && O_o->y == o_0++);
	}
	assert(!(O_o = (struct test1*) binlist_get(l, o_0/2)));
